...
- **WebSocket 상태 동기화**: 백엔드 엔진과 프론트엔드 간의 실시간 통신을 개선하여, 태스크 성공/실패 시 즉시 웹소켓 세션을 정리하고 UI를 갱신합니다.
- **데이터 무결성**: 모델 레지스트리의 보안 정책(API 키 마스킹)과 엔진의 데이터 요구사항을 분리하여, 클라우드 모델 호출 시 필요한 인증 정보가 유실되지 않도록 직접 DB 연동 아키텍처를 적용했습니다.
